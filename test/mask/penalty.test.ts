...
Deno.test("N1 counts long horizontal runs", () => {
  ...
  // override first row with five 1s
  for (let c = 0; c < size; c++) {
    matrix[0 * size + c] = 1;
  }
...
Deno.test("N3 detects horizontal finder-like pattern", () => {
  ...
  for (let i = 0; i < 11; i++) matrix[0 * size + i] = pattern[i];
...
Deno.test("N3 detects vertical finder-like pattern", () => {
  ...
  for (let i = 0; i < 11; i++) matrix[i * size + 5] = pattern[i];
...
Deno.test("penalty sums components", () => {
  ...
  for (let i = 0; i < 11; i++) matrix[2 * size + i] = pattern[i];
  matrix[5 * size + 5] = 1;
  matrix[5 * size + 6] = 1;
  matrix[6 * size + 5] = 1;
  matrix[6 * size + 6] = 1;
